...
function operation(entityId: string, payload: unknown) {
...
describe('Offline V2 — opérations idempotentes (TEST-A7-OFF)', () => {
  it('TEST-A7-OFF-01: clé d’idempotence stable, sensible au type, à l’entité et au payload', () => {
    ...
    const op = operation('r1', { note: 'boue' });
    expect(op.idempotencyKey).toBe(
      makeIdempotencyKey({
        kind: 'terrain_report',
        entityId: 'r1',
        payloadHash: hashPayload({ note: 'boue' }),
      })
    );
  ...
  it('TEST-A7-OFF-02: migration legacy — clés connues migrées, clés inconnues ignorées', () => {
    ...
    const invalid = planLegacyMigration({ [LEGACY_SYNC_QUEUE_KEY]: 'pas-du-json' });
    ...
    const reports = planLegacyMigration({
  ...
  it('TEST-A7-OFF-03: taille du pack cumulée, nulle pour un pack vide', () => {
    expect(packSizeBytes([])).toBe(0);

    const a = operation('a', { note: 'a' });
    const b = operation('b', { note: 'b' });
  ...
  it('TEST-A7-OFF-04: aucune opération en double — déduplication par clé d’idempotence', () => {
    const op = operation('same', { note: 'identique' });
  ...
  it('TEST-A7-OFF-05: métadonnées et pack d’aventure migrés vers leurs stores', () => {
    const plan = planLegacyMigration({
      lkdv_offline_pack_version: '"3"',
