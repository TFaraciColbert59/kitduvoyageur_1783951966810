...
async function waitForRecountReady(page: Page): Promise<void> {
...
test.describe('Voyage [slug] — Régression visuelle phase 3 raconter', () => {
  test('voyage-recount — /voyages/fdgb-3c3a92?phase=recount visual fidelity', async ({ page }) => {
    ...
    await page.goto('/voyages/fdgb-3c3a92?phase=recount');
    await waitForRecountReady(page);

    const buttonCount = await page.locator('button').count();
    expect(buttonCount).toBeGreaterThan(0);

    await expect(page).toHaveScreenshot('voyage-recount.png', {
      fullPage: false,
