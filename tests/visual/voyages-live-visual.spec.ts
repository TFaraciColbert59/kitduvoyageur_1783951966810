...
async function waitForLiveReady(page: Page): Promise<void> {
...
test.describe('Voyage [slug] — Régression visuelle phase 2 live', () => {
  test('voyage-live — /voyages/fdgb-3c3a92?phase=live visual fidelity', async ({ page }) => {
    await setupDeterministicEnvironment(page);
    await page.goto('/voyages/fdgb-3c3a92?phase=live');
    await waitForLiveReady(page);

    const buttonCount = await page.locator('button').count();
    expect(buttonCount).toBeGreaterThan(0);

    await expect(page).toHaveScreenshot('voyage-live.png', {
      fullPage: false,
