...
-- Contraintes idempotentes (table potentiellement pre-existante en replay).
DO $$ BEGIN
  ALTER TABLE public.group_role_delegations
    ADD CONSTRAINT group_role_delegations_window
    CHECK (ends_at > starts_at AND ends_at <= starts_at + interval '7 days');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.group_role_delegations
    ADD CONSTRAINT group_role_delegations_distinct
...
CREATE POLICY "delegations_insert_own" ON public.group_role_delegations
  ...
  WITH CHECK (
    ...
    AND (
      -- Rang de role : on ne peut deleguer que <= son propre rang.
      (delegated_role = 'observer'::public.group_member_role)
      OR (
        ...
        AND EXISTS (
          ...
          WHERE m.group_id = group_role_delegations.group_id
            ...
            AND m.status = 'active'
            AND m.role IN ('member'::public.group_member_role,
                           'co_organizer'::public.group_member_role,
                           'organizer'::public.group_member_role)
        )
      )
      OR (
        delegated_role = 'co_organizer'::public.group_member_role
        AND EXISTS (
          SELECT 1 FROM public.group_members m
          WHERE m.group_id = group_role_delegations.group_id
            AND m.user_id = auth.uid()
            AND m.status = 'active'
            AND m.role IN ('co_organizer'::public.group_member_role,
                           'organizer'::public.group_member_role)
        )
      )
      OR (
        delegated_role = 'organizer'::public.group_member_role
        AND EXISTS (
          SELECT 1 FROM public.group_members m
          WHERE m.group_id = group_role_delegations.group_id
            AND m.user_id = auth.uid()
            AND m.status = 'active'
            AND m.role = 'organizer'::public.group_member_role
        )
      )
