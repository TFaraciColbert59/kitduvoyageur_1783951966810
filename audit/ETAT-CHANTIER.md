...
|:---|:---|:---:|:---|
| **OBS-G01** | Tab bar masquant le contenu en bas d'écran | **partiel** | `src/components/shell/AppShell.tsx:75` (paddingBottom: 24px de confort). Reste à formaliser `--content-pb` calculé dans `MobilePageShell` (Lot 1 P0). |
| **OBS-G02** | Fond interrompu en bande vert uni lors du scroll | **partiel** | `src/styles/tokens.css:738-742` (classe `.g0-ambient` définie). Reste à déployer le conteneur 100dvh fixe global (Lot 1 P0). |
...
| **OBS-G07** | Formatage des nombres à l'anglo-saxonne | **partiel** | `PriceTag` et règles définies dans `COPY.md`. Audit route par route en Lot 3. |
| **OBS-G08** | Incohérence des badges Accueil | **partiel** | `Badge.tsx` prêt en verre G3. Synchronisation de la source d'état en Lot 1 P0 / Lot 2. |
...
| **OBS-G12** | Sous-onglets empilés sur la tab bar | **partiel** | Primitif `Tabs.tsx` refondu en segmented glass. Intégration de `SubTabBar` capsule en Lot 1 P0 / Lot 2. |
...
| **OBS-G16** | Accents menthe résiduels | **partiel** | Primitives UI 100% épurées. `GlobalSearchModal.tsx:319` corrigé (monochrome). Élimination des occurrences restantes sur les pages en Lot 1 P0 / Lot 3. |

---

## 3. Détail des Constats par Domaine & Page (Sections 8.1 à 8.7)

| Section | Route / Sujet | Constat & Cible | Statut | Lot Cible |
|:---|:---|:---|:---:|:---:|
...
| **8.1** | `/materiel` | Hiérarchie 4 boucliers distincts, stats au-dessus tab bar | **à faire** | Lot 1 P0 / 3 |
| **8.1** | `/kits` | Descriptions invisibles (P0) -> label sur G1, poids en SF | **à faire** | Lot 1 P0 |
...
| **8.1** | `/rapport-expedition` | Retrait PHASE 5, titre monochrome, tab bar normale (P0) | **à faire** | Lot 1 P0 |
| **8.1** | `/randonnee-active` | HUD GPS morphé sur tab bar, MapControlCluster GC (P0) | **à faire** | Lot 1 P0 |
| **8.2** | `/explorer` | 3 contrôles avec icônes (P0), attribution OSM au-dessus de `--content-pb` (P0) | **à faire** | Lot 1 P0 |
...
| **8.3** | `/communaute/carnets` | Hero photo + scrim + G1 (retrait fond menthe plein P0) | **à faire** | Lot 1 P0 |
| **8.3** | `/carnet/[slug]` | Fil d'Ariane responsive, pas de masquage sous-onglets | **à faire** | Lot 3 |
| **8.3** | Studios (expé, club, carnet) | Stepper plein écran mobile + Sheet aperçu (P0) | **à faire** | Lot 1 P0 |
| **8.3** | `/communaute/entraide` | État vide sur G1, onglet actif corrigé | **à faire** | Lot 3 |
| **8.3** | `/communaute/pro` | Garde feature flag ou état "Bientôt" (P0) | **à faire** | Lot 1 P0 |
...
| **8.3** | `/avis` | Titre monochrome lisible sur forêt (P0), Laisser un avis en haut | **à faire** | Lot 1 P0 |
...
| **8.3** | `/recompenses` | Dissociation erreur / fausses valeurs (P0), PayPal, IBAN | **à faire** | Lot 1 P0 / 3 |
| **8.4** | `/produit/[slug]` | Retrait BigBuy (P0), plateau G2 image, StickyActionBar G1 + G3 | **à faire** | Lot 1 P0 |
| **8.4** | `/location`, `/occasion` | Placeholders verre neutres, chips 12px+ | **à faire** | Lot 3 |
| **8.4** | `/panier` vide | EmptyState sur G1 | **à faire** | Lot 2 |
| **8.4** | `/checkout` | Étape 3 sur 4, affichage clair sans toucher au calcul (P0), tab bar masquée | **à faire** | Lot 1 P0 |
...
| **8.6** | `/apercu-preparation` | Garde de route (admin / dev uniquement) (P0) | **à faire** | Lot 1 P0 |
...
- [x] **Lot 0 & 1a (Actuel)** : Garde-fous de tests (`no-primary.spec.ts`, `radius.spec.ts`, `contrast.spec.ts`), tokens Liquid Glass (`tokens.css`), primitives UI de base (`Button`, `Card`, `Tabs`, `SearchField`, etc.), sélecteur d'intensité de verre et bootstrap head.
- [ ] **Captures de référence "Avant" (Section 4)** : Déploiement de `@playwright/test`, `@axe-core/playwright`, `@lhci/cli` et capture des 79 routes sous multi-viewports (390, 430, 820, 1440) en light et dark.
- [ ] **Lot 1 (P0)** : `--content-pb` calculé (OBS-G01), fond 100dvh (OBS-G02), garde `/apercu-preparation`, `/communaute/pro`, contrôles & attribution de carte, HUD rando, studios mobiles, contrastes Kits, affichage checkout.
