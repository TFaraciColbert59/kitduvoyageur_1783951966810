...
- `/configurateur` — supprimé, redirect permanent vers `/ai-configurator`
...
redirects() {
  return [
    ...
    { source: '/configurateur', destination: '/ai-configurator', permanent: true },
  ];
}
```
...
### LkvIcon
Icônes disponibles : `home`, `mountain`, `bag`, `doc`, `user`, `search`, `chevron-left`, `chevron-right`, `heart`, `bookmark`, `bell`, `map-pin`, `star`, `minus`, `plus`, `close`, `menu`, `arrow-right`, `lock`, `filter`, `users`, `compass`.
...
| Pays | `pays/[code]/CountryPageClient.tsx` | Hero drap+infos, 8 tabs scrollables, contents compressés, AI CTA cards |
...
| Terrain | `terrain/page.tsx` | Desktop redirige vers /explorer, mobile rend TerrainHub dans MobilePageShell |
...
- `CopilotFAB` — bouton flottant IA, redirige vers `/ai-configurator`
...
- `SearchOverlay` — overlay global de recherche avec recherches récentes persistées
- `TerrainHub` — hub mobile centralisant GPS, carte, kit, recherche, mode hors ligne
...
### Page /terrain
- Page statique mobile-only. Desktop redirige vers `/explorer`.
- `TerrainHub.tsx` — header avec badge GPS actif/hors ligne (`useOnlineStatus`), hero card "Naviguer" (gradient forest), grille Accès rapide 2×2 (Mon Kit, Recherche, Carte, Guides), CTA "Mode terrain persistant" (à venir).
- Badge hors ligne en ink `#6B7A72` (orange `#E4501C` interdit par le design system).

### Offline
- `OfflineBanner.tsx` — bannière sticky (z-index 55, sous safe-area) quand `navigator.onLine` est faux, toast "Connexion rétablie" au retour du réseau, icône wifi-off en SVG inline, indicateur sage `#A3C4A3`.
...
- `SearchOverlay.tsx` — overlay global (scrim + panneau glassmorphism, autofocus, recent searches persistées en localStorage).
- `useRecentSearches.ts` — hooks localStorage pour recherches récentes (add/clear/remove, max 10).
...
| Hook | Fichier | Usage |
|-|-|-|
| `useHapticFeedback` | `src/hooks/useHapticFeedback.ts` | `navigator.vibrate` — styles: light, medium, selection, success |
...
- abonnements, ambassadeurs, avis, carte-interactive, communaute, contact, evenements, explorer, pays, pro
...
- 11 slugs d'outils non implémentés supprimés (decompression, altimetre, meteo-montagne, carbone, debit-eau, pharmacie, visa, vaccins, langue, noeud, soleil)
...
Pages concernées : alertes, avis, fidelite, groupes, mes-aventures, messagerie.
