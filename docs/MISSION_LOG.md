...
## H2 — Contexte et sélecteur (08/09/2026, 21:15 UTC, plan docs/superpowers/plans/2026-09-08-h2-adventure-context.md)
- adventureSchema (cookie lkv_active_adventure) + adventureLists purs (24 tests) + GET /api/hub/adventures (groupes/crews/possession, tables existantes) + activeAdventureServer + ActiveAdventureContext (compose useActiveTrip, mémoire section/aventure, survit rechargement) + AdventureSwitcher (cmdk/GlassSheet, 3 natures, Ctrl/Cmd+K,J). TDD RED modules manquants -> GREEN.
- Revue Ive/Kare : aucune objection bloquante (liste jamais restreinte, icônes distinctes, empty states spécifiques). Garde R14 : 1 violation légitime -> adventureLists.ts en allowlist (H-AUTO-11). Axe switcher + test changement-contexte reportés H3/H7 (H-AUTO-12). Portes : G1 tsc 0 · G2 147/1134 (plancher 1056) · G3 Y-D80 12/12 + H-D85 14/14 + invariants OK. Tag : h2-done.

## H3 — Coquille et nature sortie (08/09/2026, 21:45 UTC, plan docs/superpowers/plans/2026-09-08-h3-hub-shell.md)
- hubSectionFromPathname (4 tests) + getHubAdventureData caché + buildHubCounts (API realignée) + useHubLiveSensors + TerrainShell (/terrain intact) + 5 briques (sidebars canoniques, sheet, network, picker généralisé, coreNatures au registre) + HubShell + /hub (layout/page/[section]/loading/error ×2) + provider root.
- Captures possession desktop+mobile+section inspectées HIG : 2 vrais bugs trouvés+corrigés (triggers dupliqués H-AUTO-19, hydratation network H-AUTO-20, bande widgets mobile Rams). Revue Rams/Nielsen : objection widgets mobile traitée, constance OK.
- Portes : G1 tsc 0 · G2 148/1141 (plancher 1056) · G3 Y-D80 12/12 + H-D85 14/14 + invariants OK · G4 build OK (/hub 822 B, first-load 108 kB < 250 kB) · hydration 0 erreur. Tag : h3-done.

## H4 — Possession & collectif (08/09/2026, 22:00 UTC, plan docs/superpowers/plans/2026-09-08-h4-possession-collectif.md)
- selectHubWidgets pur (4 tests) + 6 sections possession (composants+services canoniques) + 3 sections collectif (membres/rôles, invitations accept/refuse, voyages liés) + 7 widgets réels + TripSidebarRight composé (sortie). Explores parallèles (2 subagents, recherche terminale) puis exécution inline (fichiers partagés).
- Incidents : cache dev stale après rename (restart, H-AUTO-23) ; currentSeason piégé 'use client' (extraction lib, fix prod /materiel/alertes aussi, H-AUTO-24) ; R14 : 2 allowlists justifiées (H-AUTO-25). Captures inventaire/kit/alertes/collectif/groupe/invitations desktop+mobile inspectées, hydration 0. Revue Tufte/Norman : RAS bloquant.
- Portes : G1 tsc 0 · G2 149/1145 (plancher 1056) · G3 12/12+14/14+invariants OK · G4 build (/hub/[section] 343 kB first-load → watch H7). Tag : h4-done.
