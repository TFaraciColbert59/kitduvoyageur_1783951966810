...
### Lot « Assistant de préparation » — refonte produit des 6 cards (spec complète)
- **6 widgets redéfinis** (renommés + logique produit) : **Mon sac** (poids contextualisé par kit), **Prochain départ** (assistant), **Mes équipements** (essentiels couverts/manquants), **À ne pas oublier** (anticipation priorisée), **Alertes & actions** (urgent + 3 raccourcis), **Mes kits** (recommandation + complétude). `DEFAULT_WIDGET_ORDER = ['sac','departure','gear','forget','alerts','kits']` (ancien ordre localStorage invalidé → reset propre).
- **Mon sac** : poids = **kit de référence (recommandé/actif) + consommables du prochain départ** (eau/repas/gaz via `SmartDepartureEngine`) + météo ; marge vs objectif ; statut Prêt / Incomplet / Trop lourd ; fullscreen « Poids de départ estimé » (kit+consommables, catégories, plus lourds, suggestion d'allègement). Suppression du « poids universel » et de `WeightGauge`/`totalValue`/`conditionStats` devenus orphelins.
- **Prochain départ** : compact = compte à rebours, « Prêt à X% », « N chose(s) à régler », bouton primaire **« 🎒 Préparer mon départ »** ; fullscreen enrichi en **Check final** auto (prêt/chargé/entretenu/emballé/validé, coche persistée `lkdv_depart_check`).
- **Mes équipements** : couverture des **essentiels du kit de référence** (états réels : possédé / prêté / entretien / manquant) par catégorie via `mapGearCategory` (9 catégories produit) ; fallback honnête par catégorie de l'inventaire (sans dénominateur inventé) ; fullscreen avec « ✓ Ajouter à l'inventaire » pour les manquants.
- **À ne pas oublier** : **priorité stricte** (Sécurité/Indispensable > Consommables > Confort), tri critique d'abord ; actions par ligne : **✓ J'ai déjà** (coché persisté), **− Pas nécessaire** (masqué, `lkdv_forget_dismissed`), **⏰ Rappel demain** (badge, `lkdv_forget_remind`).
- **Alertes & actions** : seulement l'urgent + **3 raccourcis** (Ajouter un équipement / Créer un kit / Planifier un départ).
- **Mes kits** : bandeau **KIT RECOMMANDÉ** (compatibilité prochain départ, manquants, « Compléter / gérer ») + liste avec complétude réelle (`kitReadinessOf`).
- **Validation :** tsc 0 · lint 0 · build OK (50,1 kB) · **Playwright 26/26** (nouveaux libellés, fullscreens ×6, reorder, checklist cochée, IA drawer, mobile, 0 HTTP ≥ 400).
