...
Refonte complète de l'expérience **« Mon Matériel »** (route `/mon-materiel`) : 6 cartes en **grille 3×2** (desktop) / 1 colonne (mobile), six **vues plein écran** denses, **logique métier centralisée** dans `src/features/mon-materiel/domain` (fonctions pures), flux universel **« Ajouter à l'équipement »** (Cas A/B/C) avec **commande → réception → inventaire → kit**, design **liquid glass clair** sur fond animé Ken Burns, **zéro emoji, zéro orange `#E4501C`**, et suppression des anciennes interfaces (verify : jamais réaffichées après reload).
...
| Dépréciés / supprimés (code mort prouvé) | `src/components/cockpit/` (stubs `alert()` + données fictives), `src/hooks/useAlertsReliability.ts`, `src/lib/equipmentStatus.ts`, `src/lib/equipmentUtils.ts` (vide), `src/app/mon-materiel/page.tsx.bak`, ébauches de migration `20260819125556`/`25615` |
...
1. **À ne pas oublier** — checklist contextualisée (kit assigné + alertes + règles génériques explicitement sourcées), filtres Bloquants/À vérifier/Conseillé/Prêt, coche persistée, validation de préparation.
...
- `npm run build` : **OK** (route `/mon-materiel` 55,3 kB)
- Tests unitaires domaine (`npx tsx src/features/mon-materiel/test/domain.test.ts`) : **21/21**
- Playwright Chromium (`npx tsx scripts/pw_mon_materiel_v3.ts`) : **23/23** (grille 3×2/1-col, overflow 0, 6 fullscreens Escape/focus trap, checklist persistée, Cas A ajout kit, Cas C panier+destination, reload + hard reload **ancienne clé** → jamais d'ancienne UI, drawer, aucune erreur console)
- Sonde visuelle (`scripts/probe_visual_v3.ts`) : fond `#F5F3EE`, glass `blur(40px)`, **`hasOrange = non`**, **`emojisInTitles = non`**
...
`fix` stockage/layout → `refactor` domaine → `feat` design system → `feat` fullscreens → `feat` cockpit 3×2 → `feat` migration Supabase → `test` 21/21 + Playwright 23/23 + captures → `docs` journal.