 ...
 * navigation (même clé/version que `tests/visual/_helpers/prepareVisualPage.ts`
 * et `explorer-p1-shots.spec.ts`) + repli dismiss (clic « Refuser ») si le
 * bandeau apparaît malgré tout. Aucune capture ne doit montrer le bandeau.
 *
 * Contexte navigateur NEUF par capture (cache/état isolés).
 * Matrice minimale exigée : 393×852 · 1440×900, JPEG q72 DPR 1,
 * dans `docs/qa/final-ios27/phase-formulaires/` (+ manifest).
 * Serveur attendu : `npm run start -- --port 4028` (PW_BASE_URL)
 * avec NATIVE_TABBAR_ENABLED=false.
 */

const ROUTES = [
  { slug: 'contact', path: '/contact' },
  { slug: 'ai-configurator', path: '/ai-configurator' },
  { slug: 'outils-poids-sac', path: '/outils/poids-sac' },
  { slug: 'compte-modifier', path: '/compte/modifier' },
  { slug: 'blog', path: '/blog' },
  { slug: 'carbone', path: '/carbone' },
];
...
const OUT_DIR = path.join(process.cwd(), 'docs', 'qa', 'final-ios27', 'phase-formulaires');
...
test('P3 formulaires — captures après (consent mocké, 6 routes x 2 viewports)', async ({ browser }) => {
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, 'manifest-p3.json'),
    JSON.stringify(
      { capturedAt: new Date().toISOString(), consoleErrorCount, shots },
      null,
      2,
    ),
  );
  ...
  if (consoleErrorCount > 0) throw new Error(`${consoleErrorCount} erreur(s) console (voir manifest-p3.json)`);
});
