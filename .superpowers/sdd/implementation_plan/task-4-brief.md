# Task 4 Brief — Créer `AppShell.tsx` : source unique de vérité du layout mobile

## Context
Le Kit du Voyageur (Next.js 15 + React 19 + TypeScript). 

`MobilePageShell.tsx` (`src/components/mobile-nav/MobilePageShell.tsx`) est le composant shell actuel — il a 85 lignes, gère les safe areas et le padding bottom-nav. Il restera en service pendant la migration.

**Objectif :** Créer un nouveau composant `AppShell` qui est une version améliorée de `MobilePageShell`, avec des slots `header` et `bottomExtra`. Ce composant sera la cible finale de toutes les migrations de pages.
...
### 2. `src/components/shell/index.ts`
...
git add src/components/shell/index.ts
git commit -m "feat(shell): create AppShell — single source of truth for mobile layout, with header/bottomExtra slots"
```
...
- Ne pas modifier d'autres fichiers existants
- Ne pas dispatcher de sous-agents

## Rapport

Écrire dans `.superpowers/sdd/implementation_plan/task-4-report.md` :
