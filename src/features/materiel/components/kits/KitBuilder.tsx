...
export function KitBuilder({
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="bg-white/60 rounded-[var(--r-md)] p-3">
        ...
        <ul className="flex flex-col gap-1 max-h-[420px] overflow-y-auto">
          {inventory.map((item) => (
            <li
              ...
              className="bg-white/70 rounded-[var(--r-sm)] p-2 text-sm cursor-grab"
            >
              {item.name}
            </li>
          ))}
      ...
      <div
        className="bg-white/60 rounded-[var(--r-md)] p-3"
      ...
      >
        ...
        <Reorder.Group axis="y" values={kitItems} onReorder={setKitItems} className="flex flex-col gap-1">
          {kitItems.map((item) => (
            <Reorder.Item key={item.id} value={item} className="bg-white/70 rounded-[var(--r-sm)] p-2 text-sm">
              {item.name}
            </Reorder.Item>
          ))}
        </Reorder.Group>
