  ...
  return (
    <Card as="article" ariaLabelledBy="purchases-title" className="p-4">
      ...
      <div className="mt-4 h-[180px] w-full flex flex-col justify-between">
        {/* Bar chart area */}
        <div className="flex-1 w-full flex items-end gap-1.5 pt-4 pb-2 border-b border-[color:var(--lkv-border)]">
          {series.length > 0 ? (
            series.map((s) => {
              ...
              return (
                <div key={s.month} className="flex-1 flex flex-col items-center h-full justify-end group relative">
                  ...
                  <div className="absolute -top-7 z-[var(--z-dropdown)] opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity pointer-events-none bg-[color:var(--lkv-text-primary)] text-[color:var(--lkv-text-inverted)] text-[10px] py-0.5 px-1.5 rounded whitespace-nowrap shadow">
                    {s.valueEur} €
                  </div>
                  {/* Bar */}
                  <div
            ...
            <div className="h-full w-full flex items-center justify-center text-xs text-[color:var(--lkv-text-muted)]">
        ...
        {series.length > 0 && (
          <div className="flex items-center gap-1.5 pt-1.5">
            {series.map((s) => (
              <div key={s.month} className="flex-1 text-center truncate text-[9px] md:text-[10px] text-[color:var(--lkv-text-muted)] font-mono">
                {s.month.slice(5)}
              </div>
            ))}
          </div>
