      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {data.items.map((trail) => (
          <div key={trail.id} className="rounded-2xl border border-white/60 bg-white/70 p-4 space-y-1.5">
            <h4 className="font-display font-bold text-sm text-[color:var(--lkv-primary)] line-clamp-1">{trail.name}</h4>
            <div className="flex flex-wrap gap-x-3 gap-y-1 text-[10.5px] font-mono text-[color:var(--lkv-text-secondary)]">
              {formatDistance(trail.distanceKm) ? (
                <span className="font-bold text-[color:var(--lkv-primary-soft)]">{formatDistance(trail.distanceKm)}</span>
              ) : null}
              ...
              {trail.difficulty ? (
                <span className="text-[color:var(--lkv-secondary)] font-bold">{trail.difficulty}</span>
              ) : null}
            </div>
          </div>
        ))}
