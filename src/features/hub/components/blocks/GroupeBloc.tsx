...
export function GroupeBloc({
  trip,
  group,
}: {
  trip: { slug: string; collaborators?: unknown[] };
  group: HubCrewBlock | null;
}) {
  const collective = (group?.memberCount ?? 1) > 1 || (trip.collaborators?.length ?? 0) > 0;
  const groupeHref = tripSectionHref(trip.slug, 'team');
  ...
  if (!collective) {
    return (
      <section className="glass p-4 rounded-[var(--lkv-radius-card)]" aria-label="Groupe">
        ...
        >
          <Icon name="mail-plus" size={14} aria-hidden="true" />
  ...
  return (
    <section className="glass p-4 rounded-[var(--lkv-radius-card)]" aria-label="Groupe">
      ...
      >
        <Icon name="mail-plus" size={14} aria-hidden="true" />
        <span>{members.length === 0 ? 'Inviter un compagnon' : 'Gérer le groupe'}</span>
      </Link>
    </section>
  );
}
