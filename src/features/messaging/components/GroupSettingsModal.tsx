...
export const GroupSettingsModal: React.FC<GroupSettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white/95 backdrop-blur-2xl border border-stone-200/80 rounded-3xl max-w-md w-full p-5 text-stone-900 shadow-xl relative overflow-hidden flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="flex items-center justify-between pb-3 border-b border-stone-200/60 mb-4 shrink-0">
          <div className="flex items-center gap-2">
            <Users className="w-5 h-5 text-emerald-600" />
            <h3 className="font-bold text-base text-stone-900">Gestion du Groupe</h3>
          </div>
          <button
            onClick={onClose}
        ...
        {errorMessage && (
          <div className="mb-3 p-2.5 bg-rose-50 border border-rose-200 text-rose-800 rounded-xl text-xs font-semibold shrink-0">
            {errorMessage}
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar">
          ...
          <div className="p-3.5 bg-stone-50/90 rounded-2xl border border-stone-200/70 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full overflow-hidden relative ring-2 ring-emerald-500/20 bg-stone-200 shrink-0">
                <Image
                  src={avatarUrl || conversation.avatar_url || '/images/default-avatar.png'}
                  alt={title}
                  fill
              ...
              <div className="flex-1 overflow-hidden">
                {isEditingTitle && isAdmin ? (
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      className="w-full px-2.5 py-1 text-xs rounded-lg border border-emerald-500 bg-white font-bold text-stone-900"
                    />
                    <button
                  ...
                  <div className="flex items-center justify-between gap-1">
                    <h4 className="font-bold text-sm text-stone-900 truncate">{title}</h4>
                    {isAdmin && (
                      <button
                        onClick={() => setIsEditingTitle(true)}
                        className="p-1 rounded text-stone-400 hover:text-emerald-700 shrink-0"
                ...
                <p className="text-[11px] text-stone-500 font-medium">{members.length} membres inscrits</p>
              </div>
            </div>
            ...
            <Link
              href={expeditionUrl}
              onClick={onClose}
              className="flex items-center justify-between p-2.5 bg-emerald-50 hover:bg-emerald-100/80 border border-emerald-200/80 rounded-xl text-xs text-emerald-900 font-bold transition-all group"
            >
              <div className="flex items-center gap-2">
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                {members.map((mem) => {
                  ...
                  const avatar = mem.profile?.avatar_url || '/images/default-avatar.png';

                  return (
                    <div
                      key={mem.id}
                      className="p-2.5 bg-stone-50/80 hover:bg-stone-100/80 border border-stone-200/60 rounded-xl flex items-center justify-between gap-2"
                    >
                      <div className="flex items-center gap-2.5 overflow-hidden">
                        <div className="w-8 h-8 rounded-full overflow-hidden relative shrink-0 bg-stone-200">
                          <Image
                            src={avatar}
                            alt={name}
                            fill
                        ...
                        <div className="overflow-hidden">
                          <p className="font-bold text-xs text-stone-900 truncate flex items-center gap-1">
                            <span>{name}</span>
                            {isMe && <span className="text-[10px] text-stone-400 font-normal">(Vous)</span>}
                          </p>
                          <span
                          ...
                          >
                            {mem.role === 'owner' ? (
                              ...
                              <>
                                <Shield className="w-2.5 h-2.5 text-emerald-600" /> Admin
                              </>
                            ) : (
                              <>
                                <User className="w-2.5 h-2.5 text-stone-500" /> Membre
                              </>
                            )}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {isOwner && (
                            <button
                              ...
                              className="p-1 rounded hover:bg-stone-200 text-stone-500 text-[10px] font-semibold"
        ...
        <div className="pt-3 border-t border-stone-200/60 mt-2 shrink-0">
          <button
            ...
            disabled={loading}
            className="w-full py-2.5 px-4 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200/80 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2"
          >
