...
export function TripPhaseController({
  ...
  return (
    <div className="w-full bg-white/75 backdrop-blur-md p-1.5 rounded-2xl border border-black/5 shadow-xs">
        ...
        {phases.map(p => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col sm:flex-row items-center justify-center gap-1.5 sm:gap-2.5 py-2 sm:py-3 px-2 rounded-xl font-medium transition-all duration-200 min-h-[52px] select-none ${
                isSelected
            ...
            >
              {/* Icône de la phase */}
              <span className={isSelected ? 'text-sage-300' : 'text-lkv-secondary'}>
                {p.icon}
              </span>
