...
import { ArrowLeft, Package, Navigation, FileText } from 'lucide-react';
import { GlassSubCard } from '@/components/ui';
import type { TripFull } from '@/features/trips/types/trip.types';

interface KitSidebarLeftProps {
  trip: TripFull;
}

export default function KitSidebarLeft({ trip }: KitSidebarLeftProps) {
  const links = [
    { href: `/voyages/${trip.slug}/itineraire`, icon: Navigation, label: 'Planificateur' },
  ...
  return (
    <aside className="h-full max-h-full w-full flex-1 flex flex-col gap-3 glass rounded-[var(--lkv-radius-card)] p-3.5 text-[var(--lkv-text-primary)] font-sans overflow-y-auto no-scrollbar border border-white/40 shadow-sm select-none">
      <div className="shrink-0 space-y-2.5">
        ...
        <GlassSubCard className="p-3">
          <div className="flex items-center gap-2">
            <Package size={16} className="text-[var(--lkv-text-secondary)] shrink-0" />
            <div className="min-w-0">
              <p className="text-[9.5px] font-mono uppercase tracking-widest text-[var(--lkv-text-secondary)]">Kit & Sac a dos</p>
      ...
      <div className="mt-auto pt-2 border-t border-[var(--lkv-border-subtle)]">
        <span className="text-[8.5px] font-mono text-[var(--lkv-text-secondary)] tracking-wider uppercase">LKDV Kit Contextuel</span>
      </div>
    </aside>
  );
}