...
function InventaireTab() {
  return (
    <div className="flex flex-col gap-4 px-4 py-4">
      <Link href="/jumeau-3d" className="flex items-center gap-3 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, #1C2620 0%, #243028 100%)', color: '#E7E3D6' }} aria-label="Voir le jumeau numérique 3D">
        <div className="flex items-center justify-center rounded-xl flex-shrink-0" style={{ width: '44px', height: '44px', background: 'rgba(228,80,28,0.2)' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#17402C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /><path d="M2 12l10 5 10-5" /></svg>
        </div>
      ...
      <Link href="/inventaire" className="flex items-center gap-3 rounded-2xl p-4" style={{ background: '#EDEAE0', border: '1px solid rgba(28,38,32,0.1)' }} aria-label="Gérer mon inventaire complet">
        ...
        <div className="flex-1"><p className="font-semibold text-sm" style={{ color: '#1C2620' }}>Mon inventaire</p><p className="text-xs mt-0.5" style={{ color: '#5C6B5E' }}>Gérer tout mon équipement</p></div>
...
function MesKitsTab({ isLoggedIn }: { isLoggedIn: boolean }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center px-6 py-16 text-center gap-4">
        <div className="flex items-center justify-center rounded-full" style={{ width: '64px', height: '64px', background: 'rgba(228,80,28,0.1)' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#17402C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 10a4 4 0 0 1 4-4h8a4 4 0 0 1 4 4v10H4V10z" /><path d="M9 6V5a3 3 0 0 1 6 0v1" /></svg>
        </div>
  ...
  return (
    <div className="flex flex-col gap-4 px-4 py-4">
      <Link href="/kits" className="flex items-center gap-3 rounded-2xl p-4" style={{ background: '#EDEAE0', border: '1px solid rgba(28,38,32,0.1)' }} aria-label="Voir tous mes kits">
        <div className="flex items-center justify-center rounded-xl flex-shrink-0" style={{ width: '44px', height: '44px', background: 'rgba(228,80,28,0.1)' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#17402C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 10a4 4 0 0 1 4-4h8a4 4 0 0 1 4 4v10H4V10z" /><path d="M9 6V5a3 3 0 0 1 6 0v1" /></svg>
        </div>
        <div className="flex-1"><p className="font-semibold text-sm" style={{ color: '#1C2620' }}>Mes kits sauvegardés</p><p className="text-xs mt-0.5" style={{ color: '#5C6B5E' }}>Voir et modifier mes kits</p></div>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5C6B5E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="9 18 15 12 9 6" /></svg>
      </Link>
      <Link href="/ai-configurator" className="flex items-center gap-3 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, #17402C 0%, #cc3d10 100%)', color: 'white' }} aria-label="Créer un nouveau kit avec le configurateur IA">
        <div className="flex items-center justify-center rounded-xl flex-shrink-0" style={{ width: '44px', height: '44px', background: 'rgba(255,255,255,0.2)' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="3" /><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83" /></svg>
        </div>
        <div className="flex-1"><p className="font-semibold text-sm">Créer un kit IA</p><p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.75)' }}>Configurateur intelligent</p></div>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="9 18 15 12 9 6" /></svg>
      </Link>
    </div>
  );
}
...
function HistoriqueTab({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-3 px-4 py-4">
      {sections.map((s) => (
        <Link key={s.href} href={s.href} className="flex items-center gap-3 rounded-2xl p-4" style={{ background: '#EDEAE0', border: '1px solid rgba(28,38,32,0.1)' }} aria-label={s.label}>
          <span className="text-2xl flex-shrink-0" aria-hidden="true">{s.icon}</span>
          <div className="flex-1"><p className="font-semibold text-sm" style={{ color: '#1C2620' }}>{s.label}</p><p className="text-xs mt-0.5" style={{ color: '#5C6B5E' }}>{s.desc}</p></div>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5C6B5E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="9 18 15 12 9 6" /></svg>
        </Link>
      ))}
    </div>
...
export default function MonKitPage() {
  ...
  const tabs: { id: Tab; label: string }[] = [
    { id: 'inventaire', label: 'Inventaire' }, { id: 'mes-kits', label: 'Mes kits' }, { id: 'historique', label: 'Historique' },
  ];

  return (
    <>
      {/* DESKTOP */}
      <div className="hidden md:block" style={{ minHeight: '100vh', background: '#E7E3D6' }}>
        <div style={{ padding: '100px' }}><p style={{ textAlign: 'center', color: '#5C6B5E' }}>Vue disponible uniquement sur mobile</p></div>
      </div>
      ...
      <div className="block md:hidden">
        <MobilePageShell>
          <main id="main-content" style={{ background: '#E7E3D6', minHeight: '100dvh' }}>
            <div style={{ padding: '12px 16px', position: 'sticky', top: 0, background: 'rgba(231, 227, 214, 0.95)', backdropFilter: 'blur(8px)', zIndex: 10, borderBottom: '1px solid rgba(28,38,32,0.08)' }}>
              <div role="tablist" aria-label="Sections Mon Kit" className="flex rounded-xl p-1" style={{ background: 'rgba(28,38,32,0.08)' }}>
                {tabs.map((tab) => (
                  <button
