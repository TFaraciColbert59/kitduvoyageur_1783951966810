        ...
        <div className="w-full max-w-md">
          {success ? (
            <div className="bg-card border border-border rounded-2xl p-8 text-center">
              ...
              <h2 className="font-display text-xl text-foreground mb-2" style={{ fontFamily: 'var(--font-display)', fontWeight: 800 }}>
                Compte créé !
              </h2>
              <p className="text-sm text-foreground/60 mb-6">
                Un email de confirmation a été envoyé à <strong className="text-foreground">{form.email}</strong>. Cliquez sur le lien pour activer votre compte.
              </p>
              <Link
                href="/connexion"
            ...
            <div className="bg-card border border-border rounded-2xl p-8">
              <div className="text-center mb-8">
                <p className="text-xs font-mono text-primary tracking-widest uppercase mb-2" style={{ fontFamily: 'var(--font-mono)' }}>
                  Rejoindre la communauté
                </p>
                <h1 className="font-display text-2xl text-foreground" style={{ fontFamily: 'var(--font-display)', fontWeight: 800 }}>
                  Créer un compte
                </h1>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-foreground/70 mb-1.5">Mot de passe *</label>
                  <input
                    type="password"
                    required
                    value={form.password}
                ...
                <div>
                  <label className="block text-xs font-medium text-foreground/70 mb-1.5">Confirmer le mot de passe *</label>
                  <input
                    type="password"
                    required
                    value={form.confirm}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 disabled:opacity-60 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all mt-2"
                >
                  {loading ? (
                    <>
