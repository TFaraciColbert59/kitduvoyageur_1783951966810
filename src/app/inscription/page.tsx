        ...
        <div className="w-full max-w-md">
          {success ? (
            <div className="bg-card border border-border rounded-2xl p-8 text-center">
              <div className="w-16 h-16 rounded-full bg-[rgba(91,127,85,0.12)] flex items-center justify-center mx-auto mb-4">
                <Icon name="CheckCircleIcon" size={32} style={{ color: '#5B7F55' }} variant="outline" />
              </div>
              ...
              <Link href="/connexion" className="inline-flex items-center gap-2 bg-[#17402C] hover:bg-[#365233] text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all shadow-sm"><Icon name="ArrowRightIcon" size={14} variant="outline" />Se connecter</Link>
            </div>
          ) : (
            <div className="bg-card border border-border rounded-2xl p-8">
              <div className="text-center mb-8">
                <p className="text-xs font-mono text-[#5B7F55] tracking-widest uppercase mb-2" style={{ fontFamily: 'var(--font-mono)' }}>Rejoindre la communauté</p>
              ...
              {error && <div className="mb-4 p-3 bg-[rgba(168,68,58,0.08)] border border-[rgba(168,68,58,0.25)] rounded-xl text-[#8A241B] text-sm flex items-center gap-2"><Icon name="ExclamationCircleIcon" size={16} style={{ color: '#A8443A' }} className="flex-shrink-0" variant="outline" />{error}</div>}
              <form onSubmit={handleSubmit} className="space-y-4">
                <div><label className="block text-xs font-medium text-foreground/70 mb-1.5">Nom complet *</label><input type="text" required autoComplete="name" value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} placeholder="Jean Dupont" className="w-full px-3 py-2.5 bg-background border border-border rounded-lg text-sm text-foreground placeholder:text-foreground/30 focus:outline-none focus:border-[#5B7F55]/60 transition-colors" /></div>
                <div><label className="block text-xs font-medium text-foreground/70 mb-1.5">Email *</label><input type="email" required autoComplete="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="jean@exemple.fr" className="w-full px-3 py-2.5 bg-background border border-border rounded-lg text-sm text-foreground placeholder:text-foreground/30 focus:outline-none focus:border-[#5B7F55]/60 transition-colors" /></div>
                <div><label className="block text-xs font-medium text-foreground/70 mb-1.5">Mot de passe *</label><input type="password" required autoComplete="new-password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="8 caractères minimum" className="w-full px-3 py-2.5 bg-background border border-border rounded-lg text-sm text-foreground placeholder:text-foreground/30 focus:outline-none focus:border-[#5B7F55]/60 transition-colors" /></div>
                <div><label className="block text-xs font-medium text-foreground/70 mb-1.5">Confirmer le mot de passe *</label><input type="password" required autoComplete="new-password" value={form.confirm} onChange={(e) => setForm({ ...form, confirm: e.target.value })} placeholder="Répéter le mot de passe" className="w-full px-3 py-2.5 bg-background border border-border rounded-lg text-sm text-foreground placeholder:text-foreground/30 focus:outline-none focus:border-[#5B7F55]/60 transition-colors" /></div>
                <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 bg-[#17402C] hover:bg-[#365233] disabled:opacity-60 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all shadow-sm mt-2">
              ...
              <p className="text-center text-xs text-foreground/50 mt-6">Déjà un compte ? <Link href="/connexion" className="text-[#17402C] hover:underline font-medium">Se connecter</Link></p>
