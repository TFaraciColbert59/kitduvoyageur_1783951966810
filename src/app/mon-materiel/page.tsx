...
export default function MonMaterielPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FBFAF6] text-[#0B1F17] font-sans selection:bg-[#17402C]/10 pb-36">
      ...
      <main className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 md:pt-24 space-y-8">
          ...
          <section className="bg-white rounded-3xl p-5 sm:p-7 border border-black/[0.06] shadow-2xs space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
              ...
              <div
                ...
                className={`p-4 rounded-2xl bg-[#FBFAF6] border border-black/[0.04] flex flex-col justify-between transition-all ${
                  departurePlan.selectedKit ? 'cursor-pointer hover:border-[#17402C]/40 hover:bg-emerald-50/20 group' : ''
                }`}
              >
                <div>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <h4 className="text-sm font-bold text-[#0B1F17] group-hover:text-[#17402C] transition-colors">
            ...
            <div className="space-y-4 pt-2">
              ...
              {/* Alertes de sécurité & Équipements Recommandés */}
                ...
                <div className="space-y-2 pt-2 border-t border-black/[0.04]">
                  {departurePlan.checklist.securityChecks.map((item) => {
                    const owned = equipment.find(
                      (e) =>
                        e.name.toLowerCase().includes(item.name.toLowerCase().split(' ')[0]) ||
                        (e.category && e.category.toLowerCase() === item.category.toLowerCase())
                    );
                    const isChecked = checkedDepartureItems.has(item.id);
                    ...
                    return (
                      <div
                        key={item.id}
                        className={`p-3 rounded-2xl border text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 transition-all ${
                          isChecked
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <span className={`text-amber-950 font-medium truncate ${isChecked ? 'line-through opacity-70' : ''}`}>
                        ...
                        <div className="flex items-center gap-1.5 self-end sm:self-auto shrink-0">
                          {owned ? (
                            <button
                              type="button"
                              onClick={() => handleToggleDepartureItem(item.id)}
                              className="px-3 py-1.5 rounded-xl text-[11px] font-bold bg-[#17402C] text-white hover:bg-[#0B1F17] transition-all flex items-center gap-1"
                            >
                              <span>+ Ajouter au sac ({owned.name})</span>
                            </button>
                          ) : (
                            <>
                              <button
                                type="button"
                                onClick={() => handleQuickAddProductToInventory({
                                  name: item.name,
                                  category: item.category,
                                  weight_g: item.weightG || 120,
                                })}
                                className="px-2.5 py-1.5 rounded-xl text-[10px] font-bold bg-white hover:bg-black/[0.04] border border-black/15 text-[#0B1F17] transition-colors"
                              >
                                + J'ai déjà
                              </button>
                              <button
                                ...
                                onClick={() => handleAddToCart({
                                  ...
                                  price_eur: 24,
                                  weight_g: item.weightG || 120,
                                  brand: 'Sécurité Outdoor',
                                })}
                                className="px-2.5 py-1.5 rounded-xl text-[10px] font-bold bg-amber-900 hover:bg-amber-950 text-white transition-colors shadow-2xs"
                              >
                                🛒 Mettre au panier
                              </button>
                            </>
        ...
        <section className="space-y-4">
          ...
          <KitsManagerView
            ...
            onOpenKitCockpit={handleOpenKitCockpit}
      ...
      {/* ── 7.BIS COCKPIT DE GESTION DU KIT SÉLECTIONNÉ ── */}
      <KitCockpitDrawer
