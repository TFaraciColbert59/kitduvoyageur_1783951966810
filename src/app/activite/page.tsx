  ...
  return (
    <div className="flex flex-col gap-4 px-4 py-4">
      <Link
        ...
        aria-label="Mes carnets de voyage"
      >
        <div className="flex items-center justify-center rounded-xl flex-shrink-0" style={{ width: '44px', height: '44px', background: 'rgba(58,110,165,0.15)' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3A6EA5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z" />
            <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z" />
          </svg>
        </div>
        <div className="flex-1">
          <p className="font-semibold text-sm" style={{ color: '#1C2620' }}>Mes carnets</p>
          <p className="text-xs mt-0.5" style={{ color: '#5C6B5E' }}>Récits et photos de vos aventures</p>
        </div>
      ...
      <Link
        ...
        className="flex items-center gap-3 rounded-2xl p-4"
        style={{ background: '#EDEAE0', border: '1px solid rgba(28,38,32,0.1)' }}
        aria-label="Historique de mes sorties"
      >
        ...
        <div className="flex-1">
          <p className="font-semibold text-sm" style={{ color: '#1C2620' }}>Historique de sorties</p>
          <p className="text-xs mt-0.5" style={{ color: '#5C6B5E' }}>Toutes vos randonnées passées</p>
        </div>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5C6B5E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          <polyline points="9 18 15 12 9 6" />
        </svg>
...
function CommunauteTab() {
  const links = [
    { href: '/feed', label: 'Feed communauté', desc: 'Actualités et partages', icon: '📰', color: '#E4501C' },
    ...
    <div className="flex flex-col gap-3 px-4 py-4">
      {links.map((l) => (
        ...
        >
          ...
          <div className="flex-1">
            <p className="font-semibold text-sm" style={{ color: '#1C2620' }}>{l.label}</p>
            <p className="text-xs mt-0.5" style={{ color: '#5C6B5E' }}>{l.desc}</p>
          </div>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5C6B5E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <polyline points="9 18 15 12 9 6" />
          </svg>
...
function AmisTab({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-3 px-4 py-4">
      ...
      <p className="text-center text-xs px-4" style={{ color: '#9AAD9E' }}>
        L&apos;activité de vos amis apparaîtra ici
      </p>
    </div>
  );
}
