...
export default function ExplorerClient({ initialTrails }: ExplorerClientProps) {
  ...
  // React Query with initialData from server SSR
          ...
          <header className="flex-shrink-0 bg-white border-b border-[#E4E0D4] shadow-sm z-30">
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <span className="text-xs text-[#7A8A7D] flex-shrink-0 hidden md:block">
                {isLoading && trails.length === 0 ? '…' : `${filteredTrails.length} randonnée${filteredTrails.length !== 1 ? 's' : ''}`}
              </span>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-[#E8E4D8]">
                  <div>
                    <p className="text-sm font-semibold text-[#1C2620]">
                      {isLoading && trails.length === 0 ? 'Chargement…' : `${filteredTrails.length} randonnée${filteredTrails.length !== 1 ? 's' : ''}`}
                    </p>
                    {(!isLoading || trails.length > 0) && (
                      <p className="text-[11px] text-[#7A8A7D]">
                        {error && trails.length === 0 ? 'Erreur de chargement' : 'Triées par pertinence'}
                      </p>
                    )}
                  </div>
                  {/* Sort pill */}
        ...
        <MobilePageShell>
          ...
          >
            <span style={{ fontSize: '11px', color: '#6B7A72', fontWeight: 500 }}>
              {isLoading && trails.length === 0 ? '…' : `${filteredTrails.length} résultat${filteredTrails.length !== 1 ? 's' : ''}`}
            </span>
          </div>
