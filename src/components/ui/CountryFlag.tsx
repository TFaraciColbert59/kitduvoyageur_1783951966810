...
export default function CountryFlag({
  ...
  if (!isoCode || hasError) {
    return (
      <span
        className={`inline-flex items-center justify-center bg-[color:var(--glass-bg-medium)] border border-white/60 font-mono font-bold text-[10px] text-[color:var(--glass-label)] shadow-2xs ${sizeClasses[size]} ${className}`}
        title={name || code}
      >
