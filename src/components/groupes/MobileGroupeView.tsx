...
export default function MobileGroupeView({ data, groupId, user, members, onRefresh }: MobileGroupeViewProps) {
  ...
  return (
    <div className="md:hidden min-h-screen bg-transparent pb-36 text-lkv-primary">
      {/* IMMERSIVE COVER HERO */}
      <div className="relative w-full h-64 sm:h-72 overflow-hidden bg-lkv-primary">
        <img
          src={coverUrl}
          alt={data.meta?.titlePrefix}
          className="w-full h-full object-cover opacity-60"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-var(--lkv-primary) via-var(--lkv-primary)/60 to-transparent" />
      ...
      <div className="px-4 -mt-3 relative z-20">
        <div className="glass bg-white/90 backdrop-blur-xl p-4 rounded-3xl border border-white shadow-xs flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex -space-x-2">
                {data.travelers?.slice(0, 4).map((t: any, i: number) => (
                  <div
                    key={t.id || i}
                    className="w-8 h-8 rounded-full border-2 border-white bg-lkv-primary text-white flex items-center justify-center font-serif italic text-xs font-bold shadow-2xs"
                    style={{ zIndex: 10 - i }}
                    title={t.name}
                  >
                    {t.name?.charAt(0) || '👤'}
                ...
                {data.travelers?.length > 4 && (
                  <div className="w-8 h-8 rounded-full border-2 border-white bg-lkv-secondary text-white flex items-center justify-center font-mono font-bold text-[10px]">
                    +{data.travelers.length - 4}
                  </div>
                )}
              </div>
              ...
              <div>
                <h4 className="font-bold text-xs text-lkv-primary">
                  {data.travelers?.length || 1} co-voyageurs
                </h4>
                <span className="text-[10px] font-mono text-lkv-text-muted font-medium">
          ...
          {data.meta?.progression !== undefined && (
            <div className="pt-2 border-t border-lkv-primary/10">
              <div className="flex items-center justify-between text-[10.5px] font-mono mb-1.5 font-bold">
                <span className="text-lkv-text-muted">Préparation du sac & étapes</span>
                <span className="text-lkv-primary">{data.meta.progression}%</span>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  <div className="glass bg-white/80 p-3 rounded-2xl text-center border border-white">
                    <p className="font-mono text-[9px] uppercase tracking-widest text-lkv-text-muted mb-1 font-bold">Tâches</p>
                    <p className="font-mono font-bold text-xl text-lkv-primary">
                      {data.tasks?.filter((t: any) => !t.completed).length || 0}
                    </p>
                    <p className="text-[9px] text-lkv-text-muted font-mono">à faire</p>
                  </div>

                  <div className="glass bg-white/80 p-3 rounded-2xl text-center border border-white">
                    <p className="font-mono text-[9px] uppercase tracking-widest text-lkv-text-muted mb-1 font-bold">Budget</p>
                    <p className="font-mono font-bold text-xl text-lkv-primary">
                      {data.expenses?.perPerson || data.expenses?.total || 0} €
                    </p>
                    <p className="text-[9px] text-lkv-text-muted font-mono">par pers.</p>
                  </div>

                  <div className="glass bg-white/80 p-3 rounded-2xl text-center border border-white">
                    <p className="font-mono text-[9px] uppercase tracking-widest text-lkv-text-muted mb-1 font-bold">Matériel</p>
                    <p className="font-mono font-bold text-xl text-lkv-primary">
                      {data.equipment?.length || 0}
                    </p>
                    <p className="text-[9px] text-lkv-text-muted font-mono">équipements</p>
                  </div>
                </div>
